...
int android_close(int fd) {
  ...
  if (iter) {
    if (iter->asset) {
      ...
      return 0;
    } else if (iter->assetDir) {
      AAssetDir *assetDir = iter->assetDir;
...
int android_opendir(uv_fs_t* req) {
  const char *pathname = req->path;
  if (isPackagePath(pathname)) {
    uv_dir_t* dir;
    ...
    if (assetStat && assetStat->type == DIRECTORY_TYPE) {
      AAssetDir* assetDir = AAssetManager_openDir(android_asset_manager, subpath);
    ...
    return uv__fs_readdir(req);
  }
}

int android_readdir(uv_fs_t* req) {
  ...
  if (iter && iter->assetDir) {
    ...
    size_t asset_idx = 0;
    for (dirent_idx = 0; dirent_idx < dir->nentries; dirent_idx++) {
      uv_dirent_t* dirent = &dir->dirents[dirent_idx];
      int found = 0;

      for (; asset_idx < numAssetStats; asset_idx++) {
        AssetStat *assetStat = &assetStats[asset_idx];

        if (assetStat->parentKey == iter->key) {
          dirent->name = uv__strdup(assetStat->name);
          dirent->type = assetStat->type == FILE_TYPE ? UV_DIRENT_FILE : UV_DIRENT_DIR;
...
int android_closedir(uv_fs_t* req) {
  uv_dir_t* dir = req->ptr;
